...
app.put('/api/categories/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const updatedCategory = await Category.findOneAndUpdate({ id: parseInt(req.params.id) }, {
...
// Get single type
app.get('/api/types/:name', async (req, res) => {
    try {
...
app.post('/api/types', authMiddleware, async (req, res) => {
    try {
...
app.put('/api/types/:id', authMiddleware, async (req, res) => {
    try {
...
app.delete('/api/types/:id', authMiddleware, async (req, res) => {
    try {
        const typeId = req.params.id;
...
app.post('/api/notify-subscribers', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/unsubscribe', async (req, res) => {
    try {
        ...
        const subscriber = await Newsletter.findOneAndUpdate({ email: email.toLowerCase() }, { isActive: false }, { new: true });
        if (!subscriber) {
...
app.post('/api/orders', async (req, res) => {
    try {
        ...
        let { orderId, customerName, customerEmail, customerPhone, items, totalPrice, paymentMethod = 'Thawani', } = req.body;
        ...
        if (typeof error?.code === 'number' && error.code === 11000) {
            return res.status(409).json({ error: 'Duplicate orderId', details: error.keyValue });
        }
